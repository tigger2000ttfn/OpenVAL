# OpenVAL Installation Guide

**Document Reference:** INSTALL-001
**Version:** 1.0
**Applies To:** OpenVAL v1.x bare metal deployment on Ubuntu 22.04 LTS or RHEL 9

---

## 1. Overview

OpenVAL deploys on a single Linux server using:
...
- [ ] DNS record created: `openval.yoursite.com` pointing to the server IP
...
- [ ] Git access to clone the OpenVAL repository
...
git clone https://github.com/YOUR_ORG/openval.git /opt/openval-src
cd /opt/openval-src
...
-- Inside psql:
CREATE USER openval_app WITH PASSWORD 'CHANGE_THIS_STRONG_PASSWORD';
CREATE DATABASE openval OWNER openval_app;
\c openval
...
CREATE USER openval_readonly WITH PASSWORD 'CHANGE_THIS_READONLY_PASSWORD';
GRANT CONNECT ON DATABASE openval TO openval_readonly;
GRANT USAGE ON SCHEMA public TO openval_readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO openval_readonly;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO openval_readonly;
...
host    openval         openval_app     127.0.0.1/32            scram-sha-256
host    openval         openval_readonly 127.0.0.1/32           scram-sha-256
```
...
sudo useradd --system --shell /bin/bash --home /opt/openval --create-home openval

# Create directory structure
sudo mkdir -p /opt/openval/{app,media,logs,backups}
sudo chown -R openval:openval /opt/openval

# Create log directory for Nginx
sudo mkdir -p /var/log/openval
sudo chown openval:adm /var/log/openval
```
...
sudo -u openval git clone https://github.com/YOUR_ORG/openval.git /opt/openval/src

# Create Python virtual environment
sudo -u openval python3.11 -m venv /opt/openval/venv

# Install Python dependencies
sudo -u openval /opt/openval/venv/bin/pip install --upgrade pip
sudo -u openval /opt/openval/venv/bin/pip install -r /opt/openval/src/backend/requirements.txt

# Build frontend
cd /opt/openval/src/frontend
sudo -u openval npm ci
sudo -u openval npm run build
# Frontend build output: /opt/openval/src/frontend/dist/
```
...
sudo -u openval cp /opt/openval/src/config/.env.example /opt/openval/.env
sudo chmod 600 /opt/openval/.env
sudo chown openval:openval /opt/openval/.env

# Edit the environment file
sudo -u openval nano /opt/openval/.env
```

**Required environment variables in `/opt/openval/.env`:**
...
# ── Database ─────────────────────────────────────────
DATABASE_URL=postgresql+asyncpg://openval_app:CHANGE_THIS_STRONG_PASSWORD@localhost/openval
...
SMTP_PORT=587
SMTP_USER=openval@yoursite.com
SMTP_PASSWORD=<email-password>
SMTP_FROM_ADDRESS=openval@yoursite.com
...
# ── Logging ───────────────────────────────────────────
LOG_LEVEL=INFO
LOG_FILE=/opt/openval/logs/openval.log
...
```bash
# Run migrations
sudo -u openval /opt/openval/venv/bin/python -m alembic \
    --config /opt/openval/src/backend/alembic.ini upgrade head

# Load seed data
sudo -u openval /opt/openval/venv/bin/python \
    /opt/openval/src/scripts/seed_database.py

# Run schema Part 1 (DDL - if using raw SQL method)
sudo -u postgres psql -d openval -f /opt/openval/src/schema/openval_schema_part1.sql

# Run schema Part 2 (indexes, sequences, seed data)
sudo -u postgres psql -d openval -f /opt/openval/src/schema/openval_schema_part2.sql

# Create initial admin user
sudo -u openval /opt/openval/venv/bin/python \
    /opt/openval/src/scripts/create_admin.py \
    --username admin \
...
**`/etc/systemd/system/openval-api.service`:**
```ini
[Unit]
...
**`/etc/systemd/system/openval-worker.service`:**
```ini
[Unit]
Description=OpenVAL Celery Worker
...
ExecStart=/opt/openval/venv/bin/celery \
    ...
    --loglevel=info \
    --concurrency=4 \
    --logfile=/opt/openval/logs/celery-worker.log
KillMode=mixed
TimeoutStopSec=10
PrivateTmp=true
Restart=on-failure
...
**`/etc/systemd/system/openval-beat.service`:**
```ini
[Unit]
...
sudo systemctl enable openval-api openval-worker openval-beat
sudo systemctl start openval-api openval-worker openval-beat

# Verify all services running
sudo systemctl status openval-api openval-worker openval-beat
```

### 5.9 Nginx Configuration

**`/etc/nginx/sites-available/openval`:**
```nginx
# Redirect HTTP to HTTPS
server {
    listen 80;
    server_name openval.yoursite.com;
...
server {
    ...
    # TLS Configuration
    ssl_certificate     /etc/ssl/certs/openval.yoursite.com.crt;
    ssl_certificate_key /etc/ssl/private/openval.yoursite.com.key;
    ...
    # Logging
    access_log /var/log/openval/access.log;
    error_log  /var/log/openval/error.log;

    # Client upload size limit (match MAX_UPLOAD_SIZE_MB in .env)
    client_max_body_size 55M;

    # Serve React frontend (static files)
    root /opt/openval/src/frontend/dist;
    index index.html;
...
```bash
# Enable site
sudo ln -sf /etc/nginx/sites-available/openval /etc/nginx/sites-enabled/openval
...
```bash
sudo -u openval /opt/openval/venv/bin/python \
    /opt/openval/src/scripts/verify_installation.py
```
...
systemctl status openval-api openval-worker openval-beat nginx postgresql redis-server

# Check API health
curl -s https://openval.yoursite.com/api/v1/health | python3 -m json.tool

# Check logs for errors
sudo journalctl -u openval-api --since "5 minutes ago"
sudo tail -50 /opt/openval/logs/api-error.log
```
...
sudo -u openval bash /opt/openval/src/scripts/backup.sh

# Schedule via cron (as openval user)
sudo crontab -u openval -e
# Add:
0 2 * * * /bin/bash /opt/openval/src/scripts/backup.sh >> /opt/openval/logs/backup.log 2>&1
```

The backup script:
1. Creates a PostgreSQL dump: `pg_dump openval | gzip`
2. Archives the media directory
3. Copies the `.env` file
4. Retains last 30 days of backups
5. Logs completion or failure

**Backup location:** `/opt/openval/backups/YYYY-MM-DD/`

### Recovery

```bash
# Restore database from backup
sudo -u openval bash /opt/openval/src/scripts/restore.sh \
    --backup-dir /opt/openval/backups/2026-04-06
```
...
# 3. Pull new version
cd /opt/openval/src
sudo -u openval git fetch --tags
sudo -u openval git checkout v1.2.3

# 4. Stop services
sudo systemctl stop openval-api openval-worker openval-beat

# 5. Update Python dependencies
sudo -u openval /opt/openval/venv/bin/pip install -r backend/requirements.txt
...
sudo systemctl status openval-api openval-worker openval-beat
curl -s https://openval.yoursite.com/api/v1/health
...
```bash
# Check logs
sudo journalctl -u openval-api -n 100
sudo tail -100 /opt/openval/logs/api-error.log
...
sudo -u openval psql "postgresql://openval_app:password@localhost/openval" -c "SELECT 1"
...
```bash
# Check worker status
sudo journalctl -u openval-worker -n 100
...
sudo systemctl status openval-api
sudo ss -tlnp | grep 8000
```

---

*INSTALL-001 v1.0 - OpenVAL Bare Metal Installation Guide*
