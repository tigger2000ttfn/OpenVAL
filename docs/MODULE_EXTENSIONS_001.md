# OpenVAL Module Extensions and Enhancements
...
the current OpenVAL plan, and its target phase.
...
```
X-OpenVAL-System-Validated: true
X-OpenVAL-GxP-Context: true
X-OpenVAL-Site-Code: MATC
X-OpenVAL-API-Version: 1.0
```
...
Apply statistical process control to any numeric data series tracked in OpenVAL:
...
AI in OpenVAL is always **human-in-the-loop**. AI makes suggestions.
...
This is an optional extension module that bridges OpenVAL's quality/validation
...
### Integration with OpenVAL Quality Modules
