...
OpenVAL does not write raw SQL in application code. All database interactions
...
**MySQL note:** MySQL `DATETIME` does not store timezone info. OpenVAL normalizes
...
OpenVAL validates JSON structure at the application layer (Pydantic).
...
- The application database user (`openval_app`) physically cannot modify
...
- Append-only is enforced at the **application layer** via:
  ...
  2. The `openval_app` MySQL user is granted only `SELECT, INSERT` on
...
OpenVAL supports PostgreSQL, Oracle, and MySQL through SQLAlchemy ORM abstraction.
